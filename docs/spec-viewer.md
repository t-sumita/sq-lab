...
- **S1**: リポジトリ骨格、JSON読込+スキーマ検証、試合情報ヘッダ、
  ...
  EN/JA多言語対応
- **S2(本リリース)**: ヒートマップ描画(P1/P2 並置表示/重畳表示、
  補間による滑らかな表示、T-zone点線表示のON/OFF)
...
結果パネルのサマリーカードの下に「ヒートマップ」セクションを追加する。

- **Side by Side / Overlay** タブで表示モードを切替(モード名は指標名と
  同様に英語表記で統一)
...
- **T-zoneトグル**: metrics_params.t_zone(a_m/b_m/offset_m)から算出した
  ...
  注記と対応させて視覚的に伝える
- 正規化は **両プレイヤー共通の最大値** を用いる(片方だけ濃く見えて
  比較を誤らせないため)
